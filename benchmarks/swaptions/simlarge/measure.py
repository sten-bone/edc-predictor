...
REGEX_STRING = r"Swaption (?P<swaption>\d+): \[SwaptionPrice: (?P<price>\d+.?\d+|-?nan) StdError: (?P<error>\d+.?\d+|-?nan)\]"

def is_edc(file_out, golden_file):
    total_diff_pct = 0
    with open(golden_file, "r") as gld:
        with open(file_out, "r") as run:
            # first & second line mismatch, EDC
            if (gld.readline() + gld.readline() != run.readline() + run.readline()):
                return True
            gline = gld.readline()
            rline = run.readline()
            while (gline != "" and rline != ""):
                ...
                # fail to find pattern in run line
                if (not rmatch):
                    return True
                ...
                # mismatch swaption number is an EDC
                if (gmatch.group("swaption") != rmatch.group("swaption")):
                    return True
                
                gprice = gmatch.group("price")
                rprice = rmatch.group("price")
                if (gprice == rprice):
                    continue
                
                # if gprice is nan/-nan and rprice is not, that is too much error
                if (gprice == "nan" or gprice == "-nan"):
                    return True
                
                gld_val = float(gprice)
                run_val = float(rprice)
