...
REGEX_STRING = r"^\s+Totals:\s+(?P<current>\d+)\s+(?P<maximum>\d+)"
...
def is_edc(file_out, golden_file):
    with open(golden_file, "r") as gld:
        with open(file_out, "r") as run:
            ...
            if (not rmatch):
                return True
            gld_current = int(gmatch.group("current"))
            gld_max = int(gmatch.group("maximum"))
            run_current = int(rmatch.group("current"))
            run_max = int(rmatch.group("maximum"))
            diff_pct = max((abs(run_current - gld_current) / gld_current) * 100, (abs(run_max - gld_max) / gld_max) * 100)
            return diff_pct < 0.5
