...
def is_edc(file_out, golden_file):
    ...
    with open(golden_file, "r") as gld:
        with open(file_out, "r") as run:
            n_run = int(gld.readline()) - int(run.readline())
            # if number operated mismatch, then is EDC
            if (n_run != 0): return True
            gline = gld.readline()
            rline = run.readline()
            while (gline != "" and rline != ""):
                ...
                # if line values match, continue
                if (gld_val == run_val): continue

                # otherwise, measure percent difference from gld outline
                if (gld_val == 0):
                    total_diff_pct += abs(run_val - gld_val)
                else:
                    total_diff_pct += (abs(run_val - gld_val) / gld_val) * 100
    # definite EDC if still remaining lines in one of the files
    if (gline != "" or rline != ""):
        return True
    return total_diff_pct > 0.3
...
file_gld_out = baseline + "/prices.prof.txt"
print ("Complete.\n")
...
for f in range(0, run_count):
    ...
    file_out = prog_output + "/prices.0-" + str(f) + ".txt"
    try:
