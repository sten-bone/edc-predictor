...
REGEX_STRING = r"^iter (?P<iter>\d+), level (?P<level>\d+), residual norm (?P<resNorm>\d+.\d+)e(?P<exp>[-+]\d+), work =\s*(?P<work>\d+.\d+)"

def is_edc(file_out, golden_file):
    total_diff_pct = 0
    with open(golden_file, "r") as gld:
        with open(file_out, "r") as run:
            # first 9 lines should just match, otherwise EDC
            for i in range(9):
            ...
            while (gline != "" and rline != ""):
                ...
                # not EDC if residual norms and exponents match
                if (gmatch.group("resNorm") == rmatch.group("resNorm") and gmatch.group("exp") == rmatch.group("exp")):
                    continue
                gld_val = float(gmatch.group("resNorm"))
                run_val = float(rmatch.group("resNorm"))
                if (gld_val == 0):
